/*   arpa-e summmit demo for mosaic program
...
unsigned long offsetX = 2095000;    //tracking the starting and current absolute positions of the stages
...
bool automode = false; //Automatic movement
double incr = 0.5; //deg to step each loop
double maxZen = 60;
...
{ 
  ...
  if(Serial.available() > 0)
  {
    ...
    if(serialComm.substring(0, 6) == "zenith")
    {
      serialComm = serialComm.substring(7);
    ...
    else if(serialComm.substring(0, 7) == "xoffset")
    {
      serialComm = serialComm.substring(8);
      offsetX = mm(serialComm.toFloat()) + offsetX;
      // offsetX = serialComm.toInt();
      Serial.println("New origin x-coord confirmed");
      sendCommand(linear, axisX, moveAbs, offsetX);
    }
    else if(serialComm.substring(0, 7) == "yoffset")
    {
      serialComm = serialComm.substring(8);
      offsetY = mm(mm(serialComm.toFloat())) + offsetY;
      Serial.println("New origin y-coord confirmed");
      sendCommand(linear, axisX, moveAbs, offsetY);
    }
